...
class Evaluation(object):
    ...
    def __init__(self, splits, path_type='player_path_total'):
        ...
        for item in load_datasets(splits):
            ...
            if path_type == 'trusted_path' and 'test' not in splits:
                planner_goal = item['planner_path_total'][-1]
                if planner_goal in item['player_path_total'][1:]:
                    self.gt[item['inst_idx']]['trusted_path'] = item['player_path_total'][:]
                else:
                    self.gt[item['inst_idx']]['trusted_path'] = item['player_path_total'][:]
    ...
    def _score_item(self, instr_id, path):
        ...
        planner_goal = gt['planner_path_total'][-1]  # for calculating oracle planner success (e.g., passed over desc goal?)
        ...
        self.scores['ndtw'].append(self.ndtw(gt['scan'], path, gt['trusted_path']))
...
def eval_seq2seq():
    ...
    outfiles = [
        RESULT_DIR + "v4val-seq2seq-all-trusted_path-80-sample-imagenet_%s_iter_6000.json"
    ]
